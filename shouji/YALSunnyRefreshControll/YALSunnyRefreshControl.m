...
+ (YALSunnyRefreshControl*)attachToScrollView:(UIScrollView *)scrollView
    ...
    [refreshControl.scrollView addObserver:refreshControl forKeyPath:@"contentInset" options:NSKeyValueObservingOptionNew context:nil];
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:@"contentInset"]) {
        NSLog(@"contentInset的上间距值是： %f", self.scrollView.contentInset.top);
        return;
    }
...
-(void)calculateShift{
    ...
    NSLog(@"contentOffset的Y值是： %f", self.scrollView.contentOffset.y);
...
//    self.frame = CGRectMake(0, -(kTotalViewHeight + self.scrollView.contentInset.top), self.scrollView.frame.size.width, kTotalViewHeight);
    ...
    [self setFrame:CGRectMake(0,
                                 ...
                                 self.scrollView.frame.size.width, -self.scrollView.contentOffset.y - self.scrollView.contentInset.top)];
...
//                              self.scrollView.contentOffset.y)];
    ...
    if(self.scrollView.contentOffset.y <= (-DefaultHeight - self.scrollView.contentInset.top)){
        
        if(self.scrollView.contentOffset.y < (-SpringTreshold - self.scrollView.contentInset.top)){
            
            [self.scrollView setContentOffset:CGPointMake(0.f, (-SpringTreshold - self.scrollView.contentInset.top))];
        }
        [self scaleItems];
...
- (void)startRefreshing {
    [self.scrollView setContentOffset:CGPointMake(0.f, -DefaultHeight - self.scrollView.contentInset.top) animated:YES];
    ...
    [UIView animateWithDuration:AnimationDuration
         ...
         usingSpringWithDamping:AnimationDamping
          initialSpringVelocity:AnimationVelosity
                     ...
                     animations:^{
//                         [self.scrollView setContentInset:UIEdgeInsetsMake(64.0f, 0.f, 0.f, 0.f)];
                         [self setFrame:CGRectMake(0,
                                                   0,
                                                   self.scrollView.frame.size.width,
                                                   0)];
                         [self.scrollView setContentOffset:CGPointMake(0.f, -self.scrollView.contentInset.top)];
...
-(CGFloat)shiftInPercents{
    ...
    return (DefaultHeight / 100) * -self.scrollView.contentOffset.y - self.scrollView.contentInset.top;
}
...
-(void)scaleItems{
    ...
    if(buildigsScaleRatio <= BuildingsMaximumScale){
        ...
        CGFloat extraOffset = ABS(self.scrollView.contentOffset.y) - ABS(self.scrollView.contentInset.top) - DefaultHeight;
